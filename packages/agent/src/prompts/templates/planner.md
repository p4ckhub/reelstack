...
8. ZOOM SEGMENTS — CRITICAL FOR DYNAMIC FEEL:
...
9. B-ROLL SEARCH QUERIES: Use 1-2 word CONCRETE NOUNS ("laptop", "office desk", "smartphone"). NEVER use metaphors or abstract phrases — Pexels returns garbage for those. NEVER leave searchQuery empty.
...
10. COUNTER-UP FOR NUMBERS: Whenever the script mentions a number, percentage, price, or statistic (e.g. "73%", "5x faster", "$299", "10 000 users"), you MUST add a "counters" segment at the exact time that number is spoken. Numbers without counter-up animation look amateur. Use "rise" SFX with counters for extra impact.

11. QUALITY FIRST: Always prioritize visual quality over cost. Use the best available AI tools.
    If multiple AI video tools available, pick the best for each shot type (Seedance for cinematic, Kling for action).

12. PROMPT LANGUAGE: Always write AI image/video prompts and Pexels queries in ENGLISH, even when the script narration is in Polish or another language. AI generation models are trained predominantly on English captions and produce visibly weaker results from non-English prompts. Translate the visual concept, not the script.

## SELF-CRITIQUE (mandatory — do this BEFORE writing the JSON below)

Read your draft plan back as a muted viewer scrolling past. Answer internally:
...
List the 3 weakest beats internally. Apply a concrete fix to AT LEAST TWO of them before writing the JSON (tighter timing, swap shot type, add zoom, replace b-roll query, move or merge an effect). Do not document this self-critique in the output — just ship the improved plan.
