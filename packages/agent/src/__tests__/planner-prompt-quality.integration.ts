...
function checkPromptQuality(prompt: string, shotId: string) {
  ...
  const hasCamera =
    /\b(wide|medium|close-up|ECU|dolly|pan|tilt|orbit|crane|locked|tracking|handheld)\b/i.test(
      prompt
    );
  const hasLighting = /\b(amber|blue|warm|cool|key|rim|backlight|shadow|low-key|high-key)\b/i.test(
    prompt
  );
  const hasFraming =
    /\b(wide shot|medium shot|close-up|bird's eye|eye level|low angle|high angle)\b/i.test(prompt);
...
describe('LLM planner prompt quality', () => {
  itLive(
    'generates Seedance-compliant prompts for self-hosted script (cinematic style)',
    async () => {
      const manifest = getMockToolManifest();

      console.log(
        '\n📋 Available tools:',
        manifest.tools.map((t) => `${t.id} (${t.available ? '✓' : '✗'})`).join(', ')
      );

      const plan = await planProduction({
        script:
          'Every morning, a single line of code decides your fate. One API call to the cloud. One response that shapes your entire day. But what if you could control it all? What if your infrastructure was yours? Self-hosted. Self-owned. Self-sovereign. No vendor lock-in. No surprise bills. Just pure freedom.',
        durationEstimate: 24,
        style: 'cinematic',
      ...
      for (const shot of plan.shots) {
        ...
        if (prompt) {
          ...
          console.log(
            `    Camera: ${quality.hasCamera ? '✓' : '✗'} | Lighting: ${quality.hasLighting ? '✓' : '✗'} | Framing: ${quality.hasFraming ? '✓' : '✗'}`
          );
  ...
  itLive(
    'generates good prompts for dynamic style',
    async () => {
      const manifest = getMockToolManifest();

      const plan = await planProduction({
        script:
          "Stop paying for SaaS you don't need. Three tools. One VPS. Total control. n8n replaces Zapier. Umami replaces Google Analytics. Listmonk replaces Mailchimp. All self-hosted. All free. All yours.",
        durationEstimate: 18,
        style: 'dynamic',
        toolManifest: manifest,
      });
      ...
      for (const shot of plan.shots) {
        ...
        if (prompt) {
          const quality = checkPromptQuality(prompt, shot.id);
          console.log(
            `    ${quality.found.length > 0 ? '⚠ FORBIDDEN: ' + quality.found.join(', ') : '✓ Clean'} | Camera: ${quality.hasCamera ? '✓' : '✗'} | Words: ${quality.wordCount}`
          );
          console.log(`    Prompt: ${prompt}`);
        }
      }
