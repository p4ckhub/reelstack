...
const TEST_SCREENSHOT = 'https://upload.wikimedia.org/wikipedia/commons/thumb/4/47/PNG_transparency_demonstration_1.png/1200px-PNG_transparency_demonstration_1.png';
  ...
  sections: [
    {
      ...
      endTime: 3,
      boardType: 'bird-eye',
      kenBurns: { startScale: 1.0, endScale: 1.1, startPosition: { x: 48, y: 48 }, endPosition: { x: 52, y: 52 } },
    },
  ],
...
async function renderTest(label: string, extra: Record<string, unknown> = {}) {
  ...
  while (true) {
    ...
    if (p.fatalErrorEncountered) { console.error('FAILED:', p.errors); return; }
...
for (const f of [a, b].filter(Boolean)) {
  ...
  const info = execSync(`ffprobe -v error -show_entries stream=bit_rate -of default=noprint_wrappers=1 "${f}"`).toString().trim();
  console.log(`${f}: ${info}`);
}
